...
const Employee = () => {
  ...
  const [searchParams, setSearchParams] = useState({ name: '', salary: '', category: '' });
  ...
  const fetchData = () => {
    ...
    axios
      .get("http://localhost:3000/auth/employee", { params: queryParams })
      .then((result) => {
        if (result.data.Status) {
  ...
  const getClassById = (classIds) => {
    const classNames = classIds.map(classId => classes.find(cls => cls.id === classId)?.name);
  ...
  <div className="mt-3">
    <div className="border p-2 bg-light">
      <div className="d-flex justify-content-between align-items-center bg-info-subtle border-rounded mb-4">
        ...
        <Link to="/dashboard/add_employee" className="m-2 p-2 btn bg-primary-subtle rounded-pill shadow-sm btn-sm me-2">
      ...
      <div className="row">
        ...
        <div className="col-md-3">
          <div className="mb-2">
            ...
            <input type="text" name="salary" value={searchParams.salary} onChange={handleChange} className='border form-control rounded-pill ms-2' />
          </div>
        </div>
        <div className="col-md-3">
          <div className="mb-2"><br/>
            <button className='btn bg-success-subtle btn-sm me-2 mt-2 rounded-pill shadow-sm' onClick={() => handleSearch()}>
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-search" viewBox="0 0 16 16">
                <path d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001q.044.06.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1 1 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0" />
              </svg>
              Search
    ...
    <div className="border p-2 bg-light">
      <h3 className="text-info p-2 text-center">Information about employees </h3>
      <table className="table table-bordered text-center">
        ...
        <tbody>
          {employee.slice((currentPage - 1) * pageSize, currentPage * pageSize).map((e, i) => (
            <tr key={i}>
              <td>{e.id}</td>
              <td>
                <img
              ...
              <td>{e.name}</td>
              <td>{categories.find(category => category.id === e.category_id)?.name}</td>
              <td>{e.email}</td>
              <td>{e.address}</td>
              <td>{e.salary} DH</td>
              <td>{e.classes}</td>
              <td>
                <Link
                  ...
                  className="btn bg-primary-subtle btn-sm me-2"
                >
