...
export default function Students() {
    ...
    const [searchParams, setSearchParams] = useState({ name: '', class_id: '' });
    ...
    const fetchData = () => {
        ...
        axios
            .get("http://localhost:3000/auth/students", { params: queryParams })
            .then((result) => {
                if (result.data.Status) {
                    const modifiedStudents = result.data.Result.map(student => ({
                        ...student,
                        date_of_birth: student.date_of_birth.substring(0, 10) // Extract date part
                    }));
    ...
    const handleDelete = (id) => {
        axios.delete('http://localhost:3000/auth/delete_students/' + id)
            .then(result => {
                if (result.data.Status) {
                    fetchData();
                } else {
    ...
    const handleSearch = () => {
        setCurrentPage(1);
        fetchData();
        axios.get('http://localhost:3000/auth/students/search', { params: searchParams })
        .then((result) => { 
        ...
        <div className="mt-3">
          <div className="border p-2 bg-light">
            <div className="d-flex justify-content-between align-items-center bg-info-subtle border-rounded mb-4">
              <h4 className="p-2">Search Students</h4>
              <Link to="/dashboard/add_student" className="rounded-pill shadow-sm m-2 p-2 btn bg-primary-subtle btn-sm me-2">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-plus-circle-fill" viewBox="0 0 16 16"><path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0M8.5 4.5a.5.5 0 0 0-1 0v3h-3a.5.5 0 0 0 0 1h3v3a.5.5 0 0 0 1 0v-3h3a.5.5 0 0 0 0-1h-3z"/></svg> Add Student
              </Link>
            </div>
            <div className="row">
              ...
              <div className="col-md-3">
                <div className="mb-3">
                  ...
                  <select
                    className="form-select btn-sm me-2 rounded-pill"
                    id="class_id"
                    name="class_id"
              ...
              <div className="col-md-3">
                <div className="mt-4">
                  <button className="btn rounded-pill shadow-sm bg-success-subtle btn-sm me-2 mt-2" onClick={handleSearch}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-search" viewBox="0 0 16 16"><path d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001q.044.06.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1 1 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0"/></svg>  Search</button>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="border p-2 bg-light">
                <h3 className="text-info p-2 text-center">Information about Students </h3>
                <table className="table table-bordered text-center">
                    <thead>
                        <tr>
                            ...
                            <th className='bg-info-subtle'>Genre</th>
                            <th className='bg-info-subtle'>Address</th>
                            <th className='bg-info-subtle'>class</th>
                            <th className='bg-info-subtle'>Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        {students.slice((currentPage - 1) * pageSize, currentPage * pageSize).map((e, i) => (
                            <tr key={i}>
                                <td>{e.id}</td>
                                <td>
                                    <img
