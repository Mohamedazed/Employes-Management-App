...
const Category = () => {
    ...
    const fetchData = () => {
        ...
        axios
            .get('http://localhost:3000/auth/category', { params: queryParams })
            .then((result) => { 
                if (result.data.Status) {
                  const modifiedCategory = result.data.Result.map(cat => ({
                    ...cat,
                    created_date: cat.created_date.substring(0, 10) // Extract date part
                }));
    ...
    const handleSearch = () => {
        setCurrentPage(1); 
        fetchData();
        axios.get('http://localhost:3000/auth/category/search', { params: searchParams })
        .then((result) => { 
    ...
    return (
      <div className='px-5 mt-3'>
        <div className='mt-3'>
          <div className="border p-2 bg-light">
            <div className="d-flex justify-content-between align-items-center bg-info-subtle border-rounded mb-4">
              <h4 className="p-2">Categories List :</h4>
              <Link to="/dashboard/add_category" className="m-2 p-2 btn bg-primary-subtle btn-sm me-2 rounded-pill shadow-sm"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-plus-circle-fill" viewBox="0 0 16 16"><path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0M8.5 4.5a.5.5 0 0 0-1 0v3h-3a.5.5 0 0 0 0 1h3v3a.5.5 0 0 0 1 0v-3h3a.5.5 0 0 0 0-1h-3z"/></svg> Add Cetegory</Link>
            </div>
            <div className="row">
        ...
        <div className="border p-2 bg-light">
        <h4 className="text-info p-2 text-center">Information about categories </h4>
            <table className='table table-bordered text-center'>
                <thead>
                    <tr>
                        <th className='bg-info-subtle'>#</th>
                        <th className='bg-info-subtle'>Nom</th>
                        <th className='bg-info-subtle'>Date de creation</th>
                        <th className='bg-info-subtle'>Action</th>
                    </tr>
                </thead>
                <tbody>
                        {category.slice((currentPage - 1) * pageSize, currentPage * pageSize).map((c, i) => (
                            <tr key={i}>
                                <td>{c.id}</td>
                                <td>{c.name}</td>
